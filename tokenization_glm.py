...
class GLMTokenizerMixin:
    ...
    def build_inputs_for_generation(self, model_input: BatchEncoding, max_gen_length=512, targets=None, padding=False):
        ...
        if targets is not None:
            ...
            target_attention_mask = [[0] * seq_length + [1] * len(label) + [0] * (max_gen_length - len(label)) for label
                                     in labels]
            ...
            target_attention_mask = torch.tensor(target_attention_mask, dtype=input_ids.dtype, device=input_ids.device)
        ...
        source_attention_mask = [am.tolist() + [0] * max_gen_length for am in attention_mask]
        source_attention_mask = torch.tensor(source_attention_mask, dtype=input_ids.dtype, device=input_ids.device)
        ...
        if labels is None:
            ...
            batch["labels"] = labels
            batch["source_attention_mask"]=source_attention_mask
            batch["target_attention_mask"]=target_attention_mask
