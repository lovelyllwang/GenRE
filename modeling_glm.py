...
class GLMStack(torch.nn.Module):
    ...
    def forward(self, hidden_states, position_ids, attention_mask, memory_states=None):
        ...
        return (output,hidden_states, mem_layers)
...
class GLMModel(GLMPreTrainedModel):
    ...
    def forward(
        ...
        logits, hidden_state,hidden_layers = transformer_output
...
class GLMForConditionalGeneration(GLMPreTrainedModel):
    ...
    def _expand_inputs_for_generation(self,
        ...
        generation_attention_mask: Optional[torch.LongTensor] = None,
        ...
        **model_kwargs,
        ...
        if generation_attention_mask is not None:
            model_kwargs["generation_attention_mask"] = generation_attention_mask.index_select(0, expanded_return_idx)
        if position_ids is not None:
    ...
    def forward(
        ...
        return ModelOutput(
            loss=loss,
            logits=lm_logits,
            hidden_state=model_output.hidden_state,
            mems=model_output.mems
        )